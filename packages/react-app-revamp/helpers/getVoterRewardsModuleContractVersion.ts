...
import VoterRewardsModule from "@contracts/bytecodeAndAbi/modules/VoterRewardsModule.5.5.voterRewards.sol/VoterRewardsModule.json";
...
export async function getVoterRewardsModuleContractVersion(address: string, chainId: number) {
  ...
  const contract = new ethers.Contract(address, VoterRewardsModule.abi, provider);

  try {
    const version: string = await executeWithTimeout(MAX_TIME_TO_WAIT_FOR_RPC, contract.version());
    if (version === "5.5") {
      return VoterRewardsModule.abi;
    } else {
