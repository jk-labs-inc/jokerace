...
export const Route = createFileRoute("/contests/")({
  head: () => ({
    meta: [{ title: "Contests - jokerace" }],
  }),
  component: ContestsIndexPage,
  validateSearch: (search: Record<string, unknown>): ContestsSearchParams => {
    return {
...
function ContestsIndexPage() {
  ...
  const searchParams = useSearch({ from: "/contests/" });
  ...
  useEffect(() => {
    if (searchParams?.title && searchParams?.sortBy) {
      setSearchCriteria({
  ...
  const handleSearch = async (criteria: { query: string; filterType: string }) => {
    if (criteria.filterType === "user") {
      ...
      navigate({ to: `/user/${targetAddress}` });
    } else {
      setSearchCriteria({
