 ...
 */
function setupVoteRecipients(decimals: number, votesData: Record<string, number>): Recipient[] {
  ...
  for (const [address, votes] of Object.entries(votesData)) {
    ...
    recipients.push({
      ...
      // Scaled number of votes dedicated to the recipient
 ...
 */
function generateLeaf(address: string, numVotes: string): Buffer {
  return Buffer.from(
    // Hash in appropriate Merkle format
    solidityKeccak256(["address", "uint256"], [address, numVotes]).slice(2),
    "hex",
  );
}
 ...
 */
function createMerkleTree(recipients: Recipient[]): MerkleTree {
 ...
 */
export function createMerkleTreeFromVotes(decimals: number, votesData: Record<string, number>): MerkleTreeData {
 ...
 */
export function generateProof(merkleTree: MerkleTree, address: string, numVotes: string): Proof[] {
  // Get the leaf corresponding to the address and numVotes
