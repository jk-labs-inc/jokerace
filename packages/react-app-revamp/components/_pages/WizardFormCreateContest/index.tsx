...
export const WizardFormCreateContest = () => {
  ...
  return (
    <>
      ...
      >
        <div>
          {isReady && (!isConnected || chain?.unsupported === true) && (
            <div className="mb-5 text-sm font-bold flex items-center bg-primary-1 text-primary-10 p-3 rounded-md border-solid border border-primary-4">
              <ExclamationCircleIcon className="w-6 mie-1ex" />
              {!isConnected
                ? "Connect your wallet to create your contest."
                : chain?.unsupported === true &&
                  "We don't support this chain (yet). In the meantime, please switch to another network."}
            </div>
          )}
          <StepIndicator />
