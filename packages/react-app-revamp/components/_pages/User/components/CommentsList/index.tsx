      ...
      {comments?.count === 0 ? (
        ...
        <>
          {comments?.data.map((comment: CommentsWithContest) => (
            <UserCommentsList key={`user-comments-${comment.proposal_id}`} comment={comment} isLoading={isLoading} />
          ))}
          {comments && Math.ceil(comments.count / itemsPerPage) > 1 && (
            <Pagination
              currentPage={page}
              setCurrentPage={(newPage: number) => setPage(newPage)}
              totalPages={Math.ceil(comments.count / itemsPerPage)}
              edgePageCount={1}
